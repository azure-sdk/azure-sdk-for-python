...
class UndrainableNodeBehavior(str, Enum, metaclass=CaseInsensitiveEnumMeta):
    """Defines the behavior for undrainable nodes during upgrade.The most common cause of nodes being
    undrainable is Pod Disruption Budgets (PDBs) but other issues such as pod termination grade
    period is greater than remaining per node drain timeout or pod is still in running state can
    ...
    canceled state."""
    SCHEDULE = "Schedule"
    """AKS will still make the blocked nodes schedulable, but the blocked nodes are not upgraded. A
    best-effort attemp will be made to delete all surge nodes. The upgrade operation and the
    managed cluster will be in failed state if there is any blocked nodes."""

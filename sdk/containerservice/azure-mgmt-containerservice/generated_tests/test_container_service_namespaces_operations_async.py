...
class TestContainerServiceNamespacesOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(ContainerServiceClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_namespaces_list_by_managed_cluster(self, resource_group):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_namespaces_list_credential(self, resource_group):
        response = await self.client.namespaces.list_credential(
            resource_group_name=resource_group.name,
            resource_name="str",
            namespace_name="str",
            api_version="2025-03-02-preview",

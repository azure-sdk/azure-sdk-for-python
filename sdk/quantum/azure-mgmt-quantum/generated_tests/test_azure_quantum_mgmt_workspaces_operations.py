...
class TestAzureQuantumMgmtWorkspacesOperations(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(AzureQuantumMgmtClient)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_workspaces_check_name_availability(self, resource_group):
        response = self.client.workspaces.check_name_availability(
            location="str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_workspaces_update_tags(self, resource_group):
        response = self.client.workspaces.update_tags(
            resource_group_name=resource_group.name,
            workspace_name="str",
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_workspaces_regenerate_keys(self, resource_group):
        response = self.client.workspaces.regenerate_keys(
            resource_group_name=resource_group.name,
            workspace_name="str",
            body={"keys": ["str"]},
            api_version="2025-01-01-preview",

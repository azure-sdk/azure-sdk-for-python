...
class TestStorageActionsStorageTasksOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_storage_tasks_list(self, resource_group):
        response = self.client.storage_tasks.list(
            resource_group_name=resource_group.name,
            storage_task_name="str",
        )
        result = [r async for r in response]
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_storage_tasks_storage_task_assignment_list(self, resource_group):
        response = self.client.storage_tasks.storage_task_assignment_list(
            resource_group_name=resource_group.name,
            storage_task_name="str",
        )
        result = [r async for r in response]

...
class TestContainerServiceFleetMembersOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_fleet_members_begin_update_async(self, resource_group):
        response = await (
            await self.client.fleet_members.begin_update_async(
                resource_group_name=resource_group.name,
                fleet_name="str",
                fleet_member_name="str",
                properties={"properties": {"group": "str"}},
            )
        ).result()  # call '.result()' to poll until service return final result

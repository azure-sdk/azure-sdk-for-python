...
class TestElasticSanElasticSansOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_elastic_sans_list_by_elastic_san(self, resource_group):
        response = self.client.elastic_sans.list_by_elastic_san(
            resource_group_name=resource_group.name,
            elastic_san_name="str",
        )
        result = [r async for r in response]

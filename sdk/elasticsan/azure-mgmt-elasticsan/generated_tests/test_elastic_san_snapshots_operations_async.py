...
# --------------------------------------------------------------------------
import pytest
from azure.mgmt.elasticsan.aio import ElasticSanClient
...
class TestElasticSanSnapshotsOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(ElasticSanClient, is_async=True)
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_snapshots_list_by_volume_group(self, resource_group):
        response = self.client.snapshots.list_by_volume_group(
            resource_group_name=resource_group.name,
            elastic_san_name="str",
            volume_group_name="str",
        )

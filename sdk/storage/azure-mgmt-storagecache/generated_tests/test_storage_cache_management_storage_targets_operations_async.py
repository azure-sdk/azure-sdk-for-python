...
class TestStorageCacheManagementStorageTargetsOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    async def test_begin_dns_refresh(self, resource_group):
        response = await (
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_list_by_cache(self, resource_group):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_begin_restore_defaults(self, resource_group):
        response = await (

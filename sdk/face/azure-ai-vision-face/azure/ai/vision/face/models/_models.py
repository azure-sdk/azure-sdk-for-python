...
class CreateLivenessSessionContent(_Model):
...
class CreateLivenessWithVerifySessionContent(_Model):
    ...
    :vartype verify_image: ~azure.ai.vision.face._utils.utils.FileType
    :ivar device_correlation_id: Unique Guid per each end-user device. This is to provide rate
     limiting and anti-hammering. If 'deviceCorrelationIdSetInClient' is true in this request, this
     'deviceCorrelationId' must be null.
    :vartype device_correlation_id: str
...
class CreatePersonResult(_Model):
...
class ExposureProperties(_Model):
...
class FaceDetectionResult(_Model):
...
class FaceErrorResponse(_Model):
...
class FaceFindSimilarResult(_Model):
...
class FaceGroupingResult(_Model):
...
class FaceIdentificationCandidate(_Model):
...
class FaceIdentificationResult(_Model):
    """Identify result.
...
class FaceTrainingResult(_Model):
...
class FaceVerificationResult(_Model):
    """Verify result.
...
class LandmarkCoordinate(_Model):
...
class LargeFaceListFace(_Model):
...
class LargePersonGroupPerson(_Model):
...
class LargePersonGroupPersonFace(_Model):
...
class LivenessColorDecisionTarget(_Model):
...
class LivenessDecisionTargets(_Model):
...
class LivenessSessionAttempt(_Model):
...
class LivenessSessionResults(_Model):
...
class LivenessWithVerifyOutputs(_Model):
...
class LivenessWithVerifyReference(_Model):
...
class LivenessWithVerifyResult(_Model):
...
class LivenessWithVerifySession(_Model):
...
class LivenessWithVerifySessionAttempt(_Model):
...
class LivenessWithVerifySessionResults(_Model):
...
class OcclusionProperties(_Model):
    """Properties describing occlusions on a given face.

    ...
    async def detect(
        ...
        * Different 'recognitionModel' values are provided. If follow-up operations like Verify,
        ...
        if body is _Unset and image_content is _Unset and url is _Unset:
            raise TypeError("missing required argument: url or image_content")
        elif image_content is not _Unset and url is not _Unset:
            raise TypeError("setting url and image_content at the same time is not allowed")

        def is_json(body: Union[JSON, IO[bytes]]):
            if body is _Unset:
                return False
        ...
        if url is not _Unset or is_json(body):

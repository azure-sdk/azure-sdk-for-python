...
class TestHardwareSecurityModulesMgmtCloudHsmClustersOperations(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_begin_validate_backup(self, resource_group):
        response = self.client.cloud_hsm_clusters.begin_validate_backup(
            resource_group_name=resource_group.name,
            cloud_hsm_cluster_name="str",
            api_version="2024-06-30",
        ).result()  # call '.result()' to poll until service return final result
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_begin_validate_restore(self, resource_group):
        response = self.client.cloud_hsm_clusters.begin_validate_restore(
            resource_group_name=resource_group.name,
            cloud_hsm_cluster_name="str",
            api_version="2024-06-30",
        ).result()  # call '.result()' to poll until service return final result

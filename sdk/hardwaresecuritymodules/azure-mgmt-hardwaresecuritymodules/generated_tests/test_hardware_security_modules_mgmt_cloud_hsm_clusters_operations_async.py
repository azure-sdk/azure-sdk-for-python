...
class TestHardwareSecurityModulesMgmtCloudHsmClustersOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_begin_validate_backup(self, resource_group):
        response = await (
            await self.client.cloud_hsm_clusters.begin_validate_backup(
                resource_group_name=resource_group.name,
                cloud_hsm_cluster_name="str",
                api_version="2024-06-30",
            )
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_begin_validate_restore(self, resource_group):
        response = await (
            await self.client.cloud_hsm_clusters.begin_validate_restore(
                resource_group_name=resource_group.name,
                cloud_hsm_cluster_name="str",
                api_version="2024-06-30",
            )

...
class ApcError(_model_base.Model):
...
class Ipv4Address(_model_base.Model):
    ...
    :ivar ipv4: IPv4 address may be specified in form <address/mask> as:

     .. code-block::

              - address - an IPv4 number in dotted-quad form 1.2.3.4. Only this exact IP number
     will match the flow control rule.
              - address/mask - an IP number as above with a mask width of the form 1.2.3.4/24.
                In this case, all IP numbers from 1.2.3.0 to 1.2.3.255 will match. The bit width
     MUST be valid for the IP version. Required.
    :vartype ipv4: str
    ...
    """IPv4 address may be specified in form <address/mask> as:
     ...
     .. code-block::
              ...
              - address - an IPv4 number in dotted-quad form 1.2.3.4. Only this exact IP number will
     match the flow control rule.
              - address/mask - an IP number as above with a mask width of the form 1.2.3.4/24.
                In this case, all IP numbers from 1.2.3.0 to 1.2.3.255 will match. The bit width
     MUST be valid for the IP version. Required."""
    port: int = rest_field()
...
class Ipv6Address(_model_base.Model):
    ...
    :ivar ipv6: IPv6 address, following IETF 5952 format, may be specified in form <address/mask>
     as:
...
class NetworkIdentifier(_model_base.Model):
    ...
    :vartype identifier_type: str
    :ivar identifier: The network identifier in a format matching the type above:
...
class NumberRetrievalResult(_model_base.Model):
...
class NumberVerificationContent(_model_base.Model):

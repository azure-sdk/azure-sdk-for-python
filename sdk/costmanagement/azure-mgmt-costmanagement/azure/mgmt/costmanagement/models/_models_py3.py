...
class EAPriceSheetProperties(_serialization.Model):  # pylint: disable=too-many-instance-attributes
    ...
    :vartype effective_start_date: str
    :ivar effective_end_date: Effective end date of the Price Sheet billing period.
    :vartype effective_end_date: str
    :ivar unit_price: Price per unit at the time of billing (not the effective blended price) as
     specific to a meter and product order name.

      Note: The unit price isn't the same as the effective price in usage details downloads when
     services have differential prices across tiers. If services have multi-tiered pricing, the
     effective price is a blended rate across the tiers and doesn't show a tier-specific unit price.
     The blended price or effective price is the net price for the consumed quantity spanning across
     the multiple tiers (where each tier has a specific unit price).
    :vartype unit_price: str
    :ivar base_price: The market price at the time the customer signs on or the market price at the
     time the service meter launches if it is after sign-on.
    :vartype base_price: str
    :ivar market_price: EA list price of this service.
    ...
    :ivar price_type: Price type for a product. For example, an Azure resource with a pay-as-you-go
     rate with priceType as Consumption. Other price types include ReservedInstance, Entitlement and
     Savings Plan.
    ...
    _attribute_map = {
        ...
        "effective_start_date": {"key": "effectiveStartDate", "type": "str"},
        "effective_end_date": {"key": "effectiveEndDate", "type": "str"},
...
class MCAPriceSheetProperties(_serialization.Model):  # pylint: disable=too-many-instance-attributes
    ...
    :vartype base_price: str
    :ivar market_price: The current, prevailing market price for a given service.
    :vartype market_price: str
...
class SavingsPlanUtilizationSummary(BenefitUtilizationSummary):  # pylint: disable=too-many-instance-attributes
    ...
    :ivar avg_utilization_percentage: This is the average hourly utilization for each date range
     ...
     2023-09-01 and usageDate < 2022-10-31 at a daily granularity. There will be one record per

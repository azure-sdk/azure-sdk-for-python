...
class TestCostManagementQueryOperationsAsync(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(CostManagementClient, is_async=True)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_query_usage(self, resource_group):
        response = await self.client.query.usage(
            scope="str",
            parameters={
                "dataset": {
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_query_usage_by_external_cloud_provider_type(self, resource_group):
        response = await self.client.query.usage_by_external_cloud_provider_type(

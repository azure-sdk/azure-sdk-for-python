# coding=utf-8
# --------------------------------------------------------------------------
# Copyright (c) Microsoft Corporation. All rights reserved.
# Licensed under the MIT License. See License.txt in the project root for license information.
# Code generated by Microsoft (R) Python Code Generator.
# Changes may cause incorrect behavior and will be lost if the code is regenerated.
# --------------------------------------------------------------------------

import os

from azure.core.credentials import AzureKeyCredential

from azure.codetransparency import CodeTransparencyClient

"""
# PREREQUISITES
    pip install azure-codetransparency
# USAGE
    python get_code_transparency_config.py

    Before run the sample, please set environment variables AZURE_KEY with real value
    which can access your service
"""


def main():
    client = CodeTransparencyClient(
        endpoint="ENDPOINT",
        credential=AzureKeyCredential(key=os.getenv("AZURE_KEY")),
    )

    response = client.get_code_transparency_config()
    print(response)


# x-ms-original-file: 2024-01-11-preview/GetCodeTransparencyConfig.json
if __name__ == "__main__":
    main()

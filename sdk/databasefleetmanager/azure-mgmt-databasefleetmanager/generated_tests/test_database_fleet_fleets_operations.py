...
class TestDatabaseFleetFleetsOperations(AzureMgmtRecordedTestCase):
    def setup_method(self, method):
        self.client = self.create_mgmt_client(DatabaseFleetClient)

    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy
    def test_fleets_list(self, resource_group):
    ...
    def test_fleets_begin_update(self, resource_group):
        response = self.client.fleets.begin_update(
            resource_group_name=resource_group.name,
            fleet_name="str",
            properties={"properties": {"description": "str", "provisioningState": "str"}, "tags": {"str": "str"}},
            api_version="2025-02-01-preview",
        ).result()  # call '.result()' to poll until service return final result

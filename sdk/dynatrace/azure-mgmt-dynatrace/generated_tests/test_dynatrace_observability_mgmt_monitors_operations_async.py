...
class TestDynatraceObservabilityMgmtMonitorsOperationsAsync(AzureMgmtRecordedTestCase):
    ...
    @RandomNameResourceGroupPreparer(location=AZURE_LOCATION)
    @recorded_by_proxy_async
    async def test_monitors_update_agent_status(self, resource_group):
        response = await self.client.monitors.update_agent_status(
            resource_group_name=resource_group.name,
            monitor_name="str",
            request={
                "action": "str",
                "agentStatusList": [
                    {"dynatraceEnvironmentId": "str", "dynatraceResourceId": "str", "id": "str", "resourceType": "str"}
                ],
            },
            api_version="2024-04-24",
        )

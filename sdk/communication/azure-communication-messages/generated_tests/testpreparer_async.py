...
class ConversationAdministrationClientTestBaseAsync(AzureRecordedTestCase):

    def create_async_client(self, endpoint):
        credential = self.get_credential(ConversationAdministrationClient, is_async=True)
        return self.create_client_from_credential(
            ConversationAdministrationClient,
            credential=credential,
            endpoint=endpoint,
...
class ConversationThreadClientTestBaseAsync(AzureRecordedTestCase):

    def create_async_client(self, endpoint):
        credential = self.get_credential(ConversationThreadClient, is_async=True)
        return self.create_client_from_credential(
            ConversationThreadClient,
            credential=credential,
            endpoint=endpoint,
